...
class ReadData:
	...
	def ReadData(self, postdata):
		...
		for addrtype, addr, data, errors in self._DataKeys:
			func, addrval, errmsg = self._ParamValidator.Validate(self._PageData[addrtype], self._PageData[addr])
...
class WriteData:
	...
	def WriteData(self, postdata):
		...
		# Read the data from the field device.
		for addrtype, addr, data, errors in self._DataKeys:
			func, addrval, writevalue, errmsg = self._ParamValidator.Validate(self._PageData[addrtype], 
