...
def boollist2bin(boollist):
    ...
    try:
        return b''.join([boolhexlistinvert[tuple(boollist[i : i + 8])] for i in range(0, len(boollist), 8)])
    ...
    # If we had an error, then try again after padding it.
        ...
        if ((len(boollist) % 8) != 0):
            ...
            boolinput = boollist
        return b''.join([boolhexlistinvert[tuple(boolinput[i : i + 8])] for i in range(0, len(boolinput), 8)])
